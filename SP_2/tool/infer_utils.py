...
def draw_heatmap_EPOM(norm_cam, fields, orig_img, save_path, img_name, th):
    orig_img = torch.from_numpy(orig_img).permute(2, 0, 1).unsqueeze(0).numpy()
    for i in range(len(fields) - 1):
        k = i + 1
        gt = int(fields[k])
        hm, heatmap = draw_heatmap_array(orig_img, norm_cam[gt], th)
        cam_viz_path = os.path.join(save_path, 'heatmap', img_name + '_{}.png'.format(gt))
