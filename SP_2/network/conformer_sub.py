...
class Net(nn.Module):
    ...
    def __init__(self, patch_size=16, in_chans=3, num_classes=21, base_channel=64, channel_ratio=4, num_med_block=0,
                 ...
                 drop_rate=0.0, attn_drop_rate=0., drop_path_rate=0.1, k_cluster=10, round_nb=0):
        ...
        if round_nb == 0:
            print('======conformer_sub:======:', round_nb)
            self.trans_cls_head = nn.Linear(embed_dim, self.num_classes)
            self.conv_cls_head = nn.Conv2d(int(256 * channel_ratio), self.num_classes, kernel_size=3, stride=1,
                                           padding=1)
        else:
            print('======conformer_sub:======:', round_nb)
            self.trans_cls_head = nn.Linear(embed_dim, self.num_classes)
            self.trans_cls_head_200 = nn.Linear(embed_dim, (self.num_classes-1)*k_cluster)
            self.conv_cls_head = nn.Conv2d(int(256 * channel_ratio), self.num_classes, kernel_size=3, stride=1, padding=1)
            self.conv_cls_head_200 = nn.Conv2d(int(256 * channel_ratio), (self.num_classes-1)*k_cluster, kernel_size=3, stride=1, padding=1)
    ...
    def forward(self, method, x, round_nb):
        B = x.shape[0]
        ...
        # conv classification
        feature = F.avg_pool2d(x, kernel_size=(x.size(2), x.size(3)), padding=0)
        ...
        x_patch = x_t[:, 1:]
        n, p, c = x_patch.shape
        x_patch = torch.reshape(x_patch, [n, x_conv.shape[2], x_conv.shape[3], c])
        ...
        if round_nb == 0:
            return conv_logits, trans_logits, feature
        else:
            x_conv_200 = self.conv_cls_head_200(x)
            conv_logits_200 = self.pooling(x_conv_200).flatten(1)
            trans_logits_200 = self.trans_cls_head_200(x_t[:, 0])
            return conv_logits, trans_logits, feature, conv_logits_200, trans_logits_200
