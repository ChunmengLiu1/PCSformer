...
from tool.imutils import rescale
...
class VOC12ClsDataset(VOC12ImageDataset):
    ...
    def __init__(self, img_name_list_path, voc12_root, save_folder, round_nb, transform=None):
        ...
        self.label_200 = np.load('{}/label/R{}_train_label_200.npy'.format(save_folder, round_nb-1))
        self.label_200 = torch.from_numpy(self.label_200).float()

    def __getitem__(self, idx):
        name, img = super().__getitem__(idx)

        label_20 = torch.from_numpy(self.label_list[idx])
        label_200 = self.label_200[idx]

        return name, img, label_20, label_200

class VOC12ClsDatasetMSF(Dataset):
...
class VOC12ClsDatasetMSF_origscale(Dataset):

    def __init__(self, img_name_list_path, voc12_root, scales, inter_transform=None):
        self.img_name_list = load_img_name_list(img_name_list_path)
        self.voc12_root = voc12_root
        self.inter_transform = inter_transform
        self.label_list = load_image_label_list_from_npy(self.img_name_list)
    ...
    def __getitem__(self, idx):
        ...
        img = rescale(img)
        label = torch.from_numpy(self.label_list[idx])
        rounded_size = (int(img.size[0]), int(img.size[1]))
        ms_img_list = []
        for s in self.scales:
            target_size = (round(rounded_size[0] * s)//16*16, round(rounded_size[1] * s)//16*16)
            s_img = img.resize(target_size, resample=PIL.Image.CUBIC)
            ms_img_list.append(s_img)
