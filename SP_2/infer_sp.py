...
if __name__ == '__main__':
    ...
    parser.add_argument("--arch", default='sm_sub_infer', type=str)
    parser.add_argument("--method", default='transcam', type=str)
    parser.add_argument("--round_nb", default="1", type=int)
    parser.add_argument("--k_cluster", default=10, type=int, help="number of sub-prototype estimation")
    parser.add_argument("--save", default='./save', type=str)
    parser.add_argument("--nb_classes", default=20, type=int)
    parser.add_argument("--la", default=0.60, type=float)
    parser.add_argument("--ha", default=0.25, type=float)
    parser.add_argument("--flag_out_crf", default=False, type=bool)
    ...
    for iter, (img_name, img_list, label) in enumerate(infer_data_loader):
        ...
        if args.flag_out_crf == True:
            h, w = list(cam_dict.values())[0].shape
            tensor = np.zeros((args.nb_classes+1, h, w), np.float32)
            for key in cam_dict.keys():
                tensor[key+1] = cam_dict[key]
    
            crf_alpha = [args.la, args.ha]
            if args.out_crf is not None:
                for t in crf_alpha:
                    ...
                    out = Image.fromarray(crf.astype(np.uint8), mode='P')
                    out.putpalette(palette)
                    out_name = folder + '/' + img_name + '.png'
                    out.save(out_name)
